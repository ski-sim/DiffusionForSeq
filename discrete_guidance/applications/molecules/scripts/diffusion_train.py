...
# from run_tfbind_delta import initialize_config, setup_directories, preprocess_dataset, save_configs
# Only run as main
...
def diffusion_train(args, round_idx, dataset, cfg, orchestrator, logger, batch_off = None):
    if batch_off is not None:
        ...
        # 3. 데이터 전처리
        sequence_data_path, score_mean, score_std = preprocess_dataset(args, dataset_off, cfg)
    ...
    for model_name in train_models:
        if model_name=='denoising_model':
            ...
            # 데이터셋의 데이터 확인
            print(f"round :{round_idx},Dataset size: {len(original_dataset)}")
        ...
        trained_model = orchestrator.manager.save_model(model_name, overwrite=True)

...
from . import utils
from discrete_guidance.src import fm_utils, digress_utils
...
class DFMManager(object):
    ...
    def __init__(self, 
        ...
        for predictor_model_name, predictor_model_specs in predictor_models_dict.items():
            self.models_dict[predictor_model_name] = predictor_model_specs['model'].to(self.device)
    ...
    def load_model(self, 
        ...
        if os.path.isfile(file_path)==False:
            print(f"Could not load the '{which_model}' because no save-file could be found for this model in: {file_path}")
            return

...
class RolloutWorker:
    ...
    def guided_rollout(self, model, guide_seqs, delta):#rollout(self, model, episodes, use_rand_policy=True):
        visited = []
        lists = lambda n: [list() for i in range(n)]
        ...
        traj_rewards = lists(episodes)
        traj_dones = lists(episodes)
        padded_guide_seqs = [seq + self.eos_char * (self.max_len-len(seq))  for seq in guide_seqs]
        ...
        for t in (range(self.max_len) if episodes > 0 else []):
            ...
            ## guide
            try:
                rand = (torch.rand(len(active_indices)).to(self.device) > delta[active_indices]).int().to(self.device)
                guide = torch.tensor([self.tokenizer.stoi[padded_guide_seqs[j][t]] for j in active_indices]).to(self.device)
                guide_mask = 1 - torch.nn.functional.one_hot(guide, num_classes=21)
                mask = (1 - rand[:, None] * guide_mask).abs()
                logits = mask * logits
            except:
                import pdb; pdb.set_trace()
            try:
    ...
    def execute_train_episode_batch_with_delta(self, model, it=0, dataset=None, round=0, use_offline_data=True):
        # run an episode
        lists = lambda n: [list() for i in range(n)]
        ...
        visited, states, traj_states, \
            traj_actions, traj_rewards, traj_dones = self.guided_rollout(model, x, delta=radius)
        n = len(x)
        bulk_trajs = []
        rq = []
        for (r, mbidx) in self.workers.pop_all():
            ...
            visited.append((s, traj_rewards[mbidx][-1].item(), r.item()))
            bulk_trajs.append((s, traj_rewards[mbidx][-1].item()))
        if args.gen_data_sample_per_step > 0 and use_offline_data:
            n = args.gen_data_sample_per_step
            m = len(traj_states)
            if self.args.proxy_type == "classification":
                x, y = dataset.sample(n, 0.5)
...
def train_generator(args, generator, oracle, tokenizer, dataset, round=0):
    ...
    for it in p_bar:
        if args.radius_option == 'none':
            ...
            rollout_artifacts = rollout_worker.execute_train_episode_batch_with_delta(generator, it, dataset, round=round)
...
def sample_batch(args, rollout_worker, generator, current_dataset, oracle, dataset=None, round=0):
    ...
    while len(samples[0]) < args.num_sampled_per_round * 5:
        ...
        if args.radius_option == 'none':
            ...
            rollout_artifacts = rollout_worker.execute_train_episode_batch_with_delta(generator, it=0, dataset=dataset, round=round, use_offline_data=False)
...
def train(args, oracle, dataset):
    ...
    for round in range(args.num_rounds):
        ...
        batch = sample_batch(args, rollout_worker, generator, dataset, oracle, dataset=dataset, round=round)
